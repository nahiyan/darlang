...
process model =
    let constantPool =
        ...
        modelNew     =
            model { constantPoolBC =
                        putWord16be (intToWord16 (List.length (Types.constantPool model) + 1))
                  }
    in
...
addCPItem constantPool item =
    let constantPoolNew =
        ...
        index           =
            intToWord16 $
            List.length constantPoolNew
    in
        ( constantPoolNew, index )

addCPItems' :: ConstantPool
            -> ConstantPool
            -> [Word16]
            -> [Word16]
            -> ( ConstantPool, [Word16] )
addCPItems' constantPool items indexes stack =
    if null items
    then ( constantPool, indexes )
    else let (currentItem : restOfItems) =
                 items
...
classRef :: String
         -> ConstantPool
classRef name =
    [ ( "utf8", name ), ( "class_ref", "%x" ) ]

stringRef :: String
          -> ConstantPool
stringRef name =
    [ ( "utf8", name ), ( "string_ref", "%x" ) ]

fieldOrMethodRef :: [String]
                 -> String
                 -> ConstantPool
...
nameAndTypeBC contents =
    let segments =
        ...
        index2   =
            read (segments !! 1) :: Word16
    in
        when (List.length segments == 2) $ do
            putWord8 12
            putWord16be index1
            putWord16be index2
